...
// カレンダーイベント (calendar.json)
// type:
//  - 'school_holiday' : 学校独自の休み (祝日とは別レイヤ)
//  - 'day_swap'       : 当日は followsDay 曜日の時間割で運用 (振替・曜日指定)
//  - 'date_move'      : 当日は fromDate の時間割をここで実施 (振替・日付指定)
//  - 'period_override': 単一コマだけ差し替え or 休講
type CalendarEvent =
  | { id: string; date: string; type: 'school_holiday'; title: string; note?: string }
  | { id: string; date: string; type: 'day_swap'; followsDay: '月'|'火'|'水'|'木'|'金'; title?: string; note?: string }
  | { id: string; date: string; type: 'date_move'; fromDate: string; title?: string; note?: string }
  | {
      id: string; date: string; type: 'period_override'; period: string;
      slot: { subject: string; directory: string } | null; // null = 休講
...
* GET /api/calendar?from=YYYY-MM-DD&to=YYYY-MM-DD \=\> 範囲内の祝日（@holiday-jp/holiday_jp）と calendar.json に登録された CalendarEvent を返す。  
* POST /api/calendar \=\> CalendarEvent を新規作成。PATCH \=\> 既存イベントの更新。DELETE \=\> id 指定で削除。  
...
* **Phase 4: ダッシュボード・時間割・タスク抽出・カレンダー**  
  * **タスク**: ホーム画面の実装。timetable.json 用APIとUI。/api/tasks の実装（全ファイル走査によるTODO抽出）。calendar.json 用APIとUI（月別カレンダー画面 /calendar とホーム上の週カレンダー）。祝日は @holiday-jp/holiday_jp、休校イベント・日単位の曜日振替 (day_swap)・日付ペアでの授業移動 (date_move)・コマ単位の授業差替えを登録・反映できるようにする。  
  * **完了条件**: 時間割に科目が登録でき、ノート内の \- \[ \] がホーム画面のダッシュボードに一覧表示されること。カレンダー画面で祝日と登録した休校・振替イベントが反映され、ホームの週カレンダーにも今週分が表示されること。  
